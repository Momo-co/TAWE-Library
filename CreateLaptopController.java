 ...
 *  and primarily accepts inputs from the librarian to create a laptop.
 * @author Kevin Pan 969449
 * @author 
 * @version 1
 */
public class CreateLaptopController implements Initializable{
	...
	@FXML
	/**
	 * Reads and saves text inputs to create new laptop. 
	 * @param event The event is when the button is clicked.
	 * @throws IOException Exception to combat failed or interrupted inputs.
	 */
	public void handleSave(ActionEvent event) throws IOException{
		...
		attributes[2] = title.getText(); // name of laptop
		attributes[3] = year.getText(); // year
		attributes[4] = "laptop.jpg"; // image
		attributes[5] = manufacturer.getText(); // runtime
		attributes[6] = model.getText(); // model
		attributes[7] = os.getText(); // os
		Library.createNewResource("Laptop", attributes);
		
		Alert alert = new Alert(AlertType.INFORMATION);
		alert.setTitle("Success");
		alert.setHeaderText(null);
		alert.setContentText("Created new laptop!");
		alert.showAndWait();
	}
