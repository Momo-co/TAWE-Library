 ...
 * and outputs a list view of user item requests and allows the librarian to accept those requests.
...
public class RequestController implements Initializable{
	...
	@FXML
	private ObservableList<String> userCopies = FXCollections.observableArrayList();	
	@FXML
	...
	/**
	 * Fills the list view with the list of users who are requesting an item.
	...
	@FXML
	/**
	 * Accepts item request and checks if it is available, if so the item is ready for collection.
	...
	public void handleRequests(ActionEvent event) throws IOException{
		...
		alert.setContentText("Would you like to accept this request?");
		alert.showAndWait();
		if (alert.getResult().getText().equalsIgnoreCase("Ok")) {
			Library.reserveCopy(requestedCopy, borrower);
		}
	}
	...
	@Override
	/**
	 * Method that is immediately run when the scene is loaded and outputs data of list of users item requests.
