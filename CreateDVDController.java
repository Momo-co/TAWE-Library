 ...
 *  and primarily accepts inputs from the librarian to create a dvd.
 * @author Kevin Pan 969449
 * @author 
 * @version 1
 */

public class CreateDVDController implements Initializable{
	...
	@FXML
	/**
	 * Reads and saves text inputs to create new DVD. 
	 * @param event The event is when the button is clicked.
	 * @throws IOException Exception to combat failed or interrupted inputs.
	 */
	public void handleSave(ActionEvent event) throws IOException{
		String[] attributes = new String[9];
		attributes[0] = resID.getText();
		attributes[1] = copies.getText();
		attributes[2] = title.getText(); // name of dvd
		attributes[3] = year.getText(); // year
		attributes[4] = "cd-dvd.jpg"; // image
		attributes[5] = director.getText(); // runtime
		attributes[6] = runtime.getText(); // genre
		attributes[7] = language.getText(); // director
		attributes[8] = subtitles.getText();		
		
		Library.createNewResource("dvd", attributes);
		
		Alert alert = new Alert(AlertType.INFORMATION);
		alert.setTitle("Success");
		alert.setHeaderText(null);
		alert.setContentText("Created new DVD!");
		alert.showAndWait();
	}
