 ...
 * and outputs data of the laptop.
...
public class LaptopController implements Initializable {
	...
	@Override
	/**
	 * Method that is immediately run when the scene is loaded to output data about the laptop and its copies.
	...
	public void initialize(URL url, ResourceBundle resources) {
		...
		String tit = SearchController.selected.substring(7);
		...
		for(Resource object : Library.resourceList) {
			if (object.getTitle().trim().equalsIgnoreCase(finalTit.trim())) {
				Laptop someLaptop = (Laptop) object;
				
				year.setText(Integer.toString(someLaptop.getYear()));
				manufacturer.setText(someLaptop.getManufacturer());
				model.setText(someLaptop.getModel());
				os.setText(someLaptop.getOS());
				copies.setText(Integer.toString(someLaptop.copies.size()));
				
				for (Copy copyRef : someLaptop.copies) {
					data.add(copyRef);
				}
